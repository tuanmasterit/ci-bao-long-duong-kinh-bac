...
class Welcome extends CI_Controller {
	...
	function book($id,$row=0)
	{
		$cat = $this->Term_model->get($id,0,0,'book_cat');
		...
		else 
		{
			...
			{
				$data['is_parent'] = false;
				$data['parentCat'] = $this->Term_model->get($cat['parent'],0,0,'book_cat');
			}
			...
			include('paging.php');
			$config['per_page'] = 32;
			$config['base_url']= base_url()."/welcome/book/".$id.'/';
			$listPro = $this->Post_model->get(0,'book',0,0,'DESC','post_date',$id);
			$config['total_rows']= count($listPro);
			$config['cur_page']= $row;
			$this->pagination->initialize($config);
			$data['list_link'] = $this->pagination->create_links();
			
			$data['lstNews'] = $this->Post_model->get(0,'book',$config['per_page'],$row,'DESC','post_date',$id);
			$data['cat'] = $cat;
			
			$data['main'] = 'front_end/book';
			$this->load->view('front_end/template_2',$data);	
		}			
...
function bookdetail($id)
		{
			$news_detail = $this->Post_model->get($id, 'book', 0, 0);
			...
			{
				$data['title']='Công ty Bảo Long Đường Kinh Bắc| Sách';
